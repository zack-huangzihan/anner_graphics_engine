...
static const char *vertexShaderCode = STRINGIFY(
    precision mediump float;attribute vec3 pos; void main() { gl_Position = vec4(pos, 1.0); });

static const char *fragmentShaderCode =
    STRINGIFY(precision mediump float;uniform vec4 color; void main() { gl_FragColor = vec4(color); });
...
#else
    stbi_write_png("dbg.png", desiredWidth, desiredHeight, 3, (const void *)buffer, 0);
#endif
